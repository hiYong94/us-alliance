...
1. 작업이 PENDING이 아니면 `409 JOB_NOT_EDITABLE`
...
3. 클레임은 스케줄러와 **동일한 mutex** 를 통과 → lost update / 동시 클레임 방지
...
- 마운트 경로: `/docs`
- 스키마 소스:
  - DTO 필드: `@ApiProperty` + `class-validator` 데코레이터로부터 자동 생성
  - 컨트롤러: `@ApiOperation` (요약) + `@ApiResponse` (상태 코드별 응답 / 에러 envelope)
  - enum 값(상태 · 트리거 출처): TypeScript `enum` 정의를 그대로 노출
...
| 실패율 | `10%` (`Math.random()`) | failed 상태 시연 |
| 재시도 | 없음 | failed 영구 종결 |
...
@Cron tick
  ...
  ├─ running = true
  ├─ try
  │    jobs = repo.claimPending(BATCH_SIZE)   // mutex 안에서 pending → processing
  │    triggeredBy = 'scheduler' 로 일괄 set
  │    Promise.all(jobs.map(processOne))      // 병렬 시뮬레이션
  ├─ finally running = false

processOne(job)
  ├─ sleep(1~3s)
  ├─ if Math.random() < 0.1 → repo.markFailed(job.id, "simulated failure")
  ├─ else                    → repo.markDone(job.id)
```
...
```
controller
  ├─ repo.claimOne(id, 'manual')   // mutex 안에서 pending → processing + triggeredBy='manual'
...
### 4.2 전략 — in-process 단일 mutex

- 라이브러리: `async-mutex`
- 적용 위치: Repository 레이어 (단일 글로벌 인스턴스)
- 적용 범위:
  - read-only (`findAll`, `findOne`): **락 없음** — 메모리 캐시에서 가져옴
  - write (`create`, `update`, `claimPending`, `claimOne`, `markDone`, `markFailed`): mutex 통과

### 4.3 read-modify-write 패턴

모든 PATCH · 스케줄러 클레임 · 수동 실행 클레임은 동일한 mutex 인스턴스의 `runExclusive`
안에서 read · 검증 · write 를 완료한다.

```ts
async update(id: string, patch: PatchDto): Promise<Job> {
  return this.mutex.runExclusive(async () => {
    const job = await this.readOne(id);
    if (!job || job.deletedAt) throw new JobNotFoundException();
...
- 환경변수 `LOG_DIR` 로 디렉토리 오버라이드 (테스트 격리)

### 5.2 로그 항목 (예시)

```jsonc
// HTTP 요청 (성공)
{ "ts": "...", "level": "info", "type": "http", "method": "POST", "path": "/jobs", "status": 201, "durationMs": 12, "requestId": "..." }

// HTTP 에러
{ "ts": "...", "level": "error", "type": "http", "method": "PATCH", "path": "/jobs/abc", "status": 409, "durationMs": 3, "code": "JOB_NOT_EDITABLE", "requestId": "..." }

// 스케줄러 tick
{ "ts": "...", "level": "info", "type": "scheduler", "event": "tick.start", "claimed": 5 }
{ "ts": "...", "level": "info", "type": "scheduler", "event": "job.done", "jobId": "...", "durationMs": 1834, "triggeredBy": "scheduler" }
{ "ts": "...", "level": "warn", "type": "scheduler", "event": "job.failed", "jobId": "...", "durationMs": 2100, "reason": "simulated failure", "triggeredBy": "scheduler" }
{ "ts": "...", "level": "info", "type": "scheduler", "event": "tick.end", "processed": 5, "failed": 1 }
```
...
| 요청 추적 ID | Middleware + AsyncLocalStorage | `requestId` 미들웨어 + `@RequestId()` 커스텀 데코레이터 |
| 락 | 라이브러리 네이티브 API | `mutex.runExclusive` (Repository 레이어) |
...
| 계층 | 도구 | 대상 |
|---|---|---|
| 단위 | Jest | 도메인 로직 (상태 전이, 검색 필터링, mutex 동작), 헬퍼 |
...
- 데이터: 각 테스트는 임시 디렉토리에 `jobs.test.json` 생성, `JOBS_DB_PATH` 주입
- 로그: 동일하게 `LOG_DIR` 주입, 테스트 후 정리
- 결정성: `Math.random` 모킹 (`jest.spyOn(global.Math, 'random')`) 으로 실패율 검증
